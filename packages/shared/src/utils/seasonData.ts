 ...
 *
 * Denna modul innehåller bara synkrona funktioner - plattformsspecifika
 * async wrappers finns i respektive paket (web/app).
 */

import type { Zone, TimelineZone, Category, Crop } from '../data/types'
...
/** Synkron version - kräver att allCrops skickas in som parameter */
export function getSeasonActivitiesSync(month: number, zone: Zone, allCrops: Record<string, Crop>): SeasonGroup[] {
  return buildSeasonGroups(month, zone, allCrops)
}

function buildSeasonGroups(month: number, zone: Zone, CROPS: Record<string, Crop>): SeasonGroup[] {
